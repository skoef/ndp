...
// As defined in https://tools.ietf.org/html/rfc4861#section-4.5
...
func (p *ICMPRedirect) String() string {
	m, _ := p.Marshal()
	s := fmt.Sprintf("%s, length %d, ", p.Type(), uint8(len(m)))
	s += fmt.Sprintf("redirect %s to %s\n", p.TargetAddress, p.DestinationAddress)
	for _, o := range p.Options {
		s += fmt.Sprintf("    %s\n", o)
	}
...
func (p *ICMPRedirect) Marshal() ([]byte, error) {
	...
	// b[2:3] = checksum, TODO
	b = append(b, p.TargetAddress...)
	b = append(b, p.DestinationAddress...)
	// add options
	om, err := p.optMarshal()
	if err != nil {
		return nil, err
